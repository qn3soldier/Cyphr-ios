...
import CyphrPinModal from '@/components/auth/CyphrPinModal';
import CyphrBiometryModal from '@/components/auth/CyphrBiometryModal';
...
export default function Welcome() {
  ...
  useEffect(() => {
    if (!isNewUser && loginMethod === 'cyphr_identity') {
      console.log('🔍 Auto-triggering Cyphr Identity login...');
      handleCyphrIdLogin();
    }
  }, [loginMethod, isNewUser]);
  ...
  const checkForStoredCredentials = async () => {
    try {
      ...
      if (userId) {
        console.log('✅ Found existing session, navigating to chats');
        navigate('/chats');
        return;
      }

      // Check for stored device credentials
      console.log('🔍 Attempting to get stored device credentials...');
      const credentialResult = await cryptoAuth.getStoredDeviceCredentials();
      console.log('🔍 Stored credentials result:', credentialResult);
      ...
      if (credentialResult.success) {
        ...
        if (statusResult.success) {
          ...
          if (!statusResult.hasPin && !statusResult.hasBiometry) {
            // Direct login without additional security
  ...
  const performAutoLogin = async (credentialData) => {
    try {
      ...
      sessionStorage.setItem('userId', credentialData.cyphrId);
  ...
  const handleCyphrIdLogin = async () => {
    ...
    console.log('🚀 DEBUG: isNewUser =', isNewUser, 'loginMethod =', loginMethod);
    ...
    try {
      ...
      console.log('🔍 DEBUG: About to call getStoredDeviceCredentials...');
      
      // Try to get stored device credentials
      const credentialResult = await cryptoAuth.getStoredDeviceCredentials();
      console.log('🔍 DEBUG: credentialResult =', credentialResult);
      ...
      if (credentialResult.success) {
        ...
        // No stored credentials found - НЕ показываем toast.error сразу!
        console.log('ℹ️ No stored Cyphr ID credentials found');
        
        // Вместо ошибки, показываем информацию пользователю
      ...
      toast.error('Cyphr ID login failed');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    try {
      if (loginMethod === 'phone') {
        ...
        if (isNewUser) {
          ...
          // Existing users: Auto-login is handled by useEffect
          // This section should not be reached due to auto-trigger
          console.log('ℹ️ Login users auto-login is handled by useEffect');
        }
      }
    } catch (err) {
    ...
    return (
      <div className="relative min-h-screen w-full bg-gradient-to-b from-[#1a1a2e] to-[#16213e] text-white flex items-center justify-center p-4 overflow-hidden font-sans">
        <div className="text-center">
          ...
          />
          <p className="text-gray-400 mt-4">Checking for stored credentials...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="relative min-h-screen w-full bg-gradient-to-b from-[#1a1a2e] to-[#16213e] text-white flex items-center justify-center p-4 overflow-hidden font-sans">
      ...
      >
        <form onSubmit={handleSignIn} className="flex flex-col items-center">
          ...
          {/* Only show main button for non-Cyphr Identity methods OR new users */}
          ...
          {/* Cyphr ID Login Option - Only for Login and non-Cyphr Identity methods */}
          {!isNewUser && loginMethod !== 'cyphr_identity' && (
      ...
      {/* 🔐 Cyphr Identity PIN Modal */}
      <CyphrPinModal
        isOpen={showCyphrPinModal}
        onClose={() => setShowCyphrPinModal(false)}
        cyphrUserData={cyphrUserData}
        onSuccess={handleCyphrAuthSuccess}
      />

      {/* 👆 Cyphr Identity Biometry Modal */}
      <CyphrBiometryModal
        isOpen={showCyphrBiometryModal}
        onClose={() => setShowCyphrBiometryModal(false)}
        cyphrUserData={cyphrUserData}
        onSuccess={handleCyphrAuthSuccess}
      />
    </div>
