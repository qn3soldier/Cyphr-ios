...
class ZeroKnowledgeAuth {
  ...
  async checkCyphrIdStatus(cyphrId) {
    try {
      console.log('🔍 Checking Cyphr ID status:', cyphrId);
      
      // Remove @ prefix if provided
      const cleanCyphrId = cyphrId.startsWith('@') ? cyphrId.slice(1) : cyphrId;
      
      const apiBase = (import.meta.env.VITE_BACKEND_URL || 'https://app.cyphrmessenger.app/api').replace(/\/$/, '');
      const response = await fetch(`${apiBase}/auth/check-cyphr-id-status`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ cyphrId: cleanCyphrId })
      });

      const data = await response.json();
      console.log('🔍 Cyphr ID status response:', data);

      if (response.ok && data.success) {
        return {
          success: true,
          cyphrId: data.cyphrId,
          userId: data.userId,
          hasPin: data.hasPin,
        ...
        return {
          success: false,
          error: data.error || 'Failed to check Cyphr ID status'
        };
      }
    } catch (error) {
  ...
  async cyphrIdLogin(cyphrId, pin) {
    try {
