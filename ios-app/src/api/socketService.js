...
class SocketService {
  constructor() {
    ...
    // iOS-specific WebSocket suspension handling
    ...
    // this.initializeP2P();
    this.setupiOSSuspensionHandling();
  }
  ...
  setupiOSSuspensionHandling() {
    // Handle iOS app state changes (background/foreground)
    this.appStateSubscription = AppState.addEventListener('change', (nextAppState) => {
      if (nextAppState === 'background') {
        console.log('📱 iOS app backgrounded - preparing for suspension');
        this.backgroundTime = Date.now();
        this.handleiOSSuspension();
      } else if (nextAppState === 'active') {
        console.log('📱 iOS app foregrounded - resuming connections');
        
        if (this.backgroundTime && (Date.now() - this.backgroundTime > 30000)) {
          // App was suspended for >30 seconds, force reconnect
          console.log('🔄 Long iOS suspension detected, reconnecting...');
          this.handleiOSLongSuspension();
        } else {
          this.handleiOSResume();
        }
  ...
  handleiOSSuspension() {
    ...
    if (this.socket) {
      this.socket.emit('app_suspended', { 
        timestamp: Date.now(),
        platform: 'ios' 
      });
    }
    
    // Stop heartbeat to conserve battery
    if (this.heartbeatInterval) {
      clearInterval(this.heartbeatInterval);
      this.heartbeatInterval = null;
    }
  ...
  handleiOSResume() {
    ...
    if (!this.isConnected && this.socket) {
      console.log('🔄 Attempting to reconnect after iOS resume...');
      this.connect();
    }
  }

  async handleiOSLongSuspension() {
    // iOS app was suspended for >30 seconds
    console.log('🔄 iOS long suspension detected, forcing reconnect...');
  ...
  async showiOSPushNotification(message) {
    try {
      // Show iOS push notification when app is backgrounded
      await Notifications.scheduleNotificationAsync({
        content: {
          title: `${message.sender_name || 'New Message'}`,
          body: message.decryptedContent?.substring(0, 100) || message.content?.substring(0, 100),
          sound: 'default',
          badge: 1,
        },
      ...
      console.error('❌ iOS push notification failed:', error);
    }
  }
