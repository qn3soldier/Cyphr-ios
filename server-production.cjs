...
async function initializeServices() {
    try {
        // Initialize Kyber1024 encryption
        ...
        const rdsConnected = await rdsService.initialize();
        if (rdsConnected) {
            console.log('✅ AWS RDS PostgreSQL connected');
        } else {
            console.warn('⚠️ RDS connection failed, falling back to Supabase');
        }
    }
    catch (error) {
...
// Database abstraction layer - use RDS when available, fallback to Supabase
async function dbQuery(table, operation, data = {}, filters = {}, customSql = null) {
    if (rdsService && rdsService.isConnected) {
        ...
        if (customSql && customSql.fallbackSupabase) {
            // Handle custom Supabase queries for complex operations
            return await customSql.fallbackSupabase(adminSupabase);
        }
        ...
        switch (operation) {
            ...
            case 'insert':
                return await adminSupabase.from(table).insert(data).select().single();
                
            case 'update':
                let updateQuery = adminSupabase.from(table).update(data);
                Object.entries(filters).forEach(([key, value]) => {
                    updateQuery = updateQuery.eq(key, value);
                });
                return await updateQuery.select().single();
                
            case 'delete':
                let deleteQuery = adminSupabase.from(table).delete();
                Object.entries(filters).forEach(([key, value]) => {
    ...
    try {
        ...
        if (verificationCheck.status === 'approved') {
            ...
            try {
                ...
                const { data: existingUserResult } = await dbQuery('users', 'select', {}, { phone: normalizedPhone });
                const existingUser = existingUserResult && existingUserResult.length > 0 ? existingUserResult[0] : null;
                let user;
                if (existingUser) {
                    user = existingUser;
                    ...
                    const { data: newUser, error: userError } = await dbQuery('users', 'insert', {
                        id: userId,
                        phone: normalizedPhone,
...
// Email OTP storage (in production use Redis)
...
app.post('/api/auth/send-email-otp', async (req, res) => {
    try {
        ...
        // Check if this email already has account (one email = one account)
        const userCheck = await dbQuery('users', 'customSql', {}, {}, {
            query: 'SELECT COUNT(*) as user_count FROM users WHERE email_hash = $1',
            params: [crypto_1.default.createHash('sha256').update(email).digest('hex')]
        });

        if (userCheck.data && userCheck.data[0].user_count > 0) {
        ...
        // Store OTP
        emailOTPCodes.set(email, { otp, expiresAt });

        // Send email via AWS SES
        const emailContent = {
            Source: 'insomnianclub@gmail.com', // Verified email
            Destination: { ToAddresses: [email] },
            Message: {
                Subject: { 
                    Data: 'Your Cyphr Messenger Verification Code',
                    Charset: 'UTF-8'
                },
                Body: {
                    Html: {
                        ...
                        <div style="font-family: Arial; max-width: 600px; margin: 0 auto;">
                            <h2>🔐 Your Cyphr Messenger Verification Code</h2>
                            <p>Enter this code to complete your authentication:</p>
                            <div style="background: #f5f5f5; padding: 20px; text-align: center; font-size: 32px; font-weight: bold; color: #333; margin: 20px 0;">${otp}</div>
...
app.post('/api/auth/verify-email-otp', async (req, res) => {
    try {
        const { email, otp, code, isSignUp = false } = req.body;
        ...
        // Check stored OTP
        const storedOTP = emailOTPCodes.get(email);
        ...
        if (isSignUp) {
            // Create new user in RDS with device binding
            const userId = crypto_1.default.randomUUID();
            const userData = {
                id: userId,
                email_hash: crypto_1.default.createHash('sha256').update(email).digest('hex'),
                auth_method: 'email',
                created_at: new Date().toISOString(),
                updated_at: new Date().toISOString()
            };

            const result = await dbQuery('users', 'insert', userData);
...
// Load PIN Authentication Endpoints
const { createPinAuthEndpoints } = require("./pin-auth-endpoints.cjs");
createPinAuthEndpoints(app, supabase, null, JWT_SECRET);
console.log("u2705 PIN Authentication endpoints loaded");
