  ...
  const loadProfile = async () => {
    try {
      ...
      if (data) {
        setProfile({
          id: data.id,
          name: data.name || userName || `User ${userPhone?.slice(-4)}`,
          bio: data.bio || 'Hey there! I am using Cyphr Messenger',
          phone: data.phone || userPhone,
          avatar_url: data.avatar_url || '',
          public_key: data.public_key || userPublicKey,
          cyphr_id: data.cyphr_id || ''
        });
        setEditedName(data.name || userName || `User ${userPhone?.slice(-4)}`);
        setEditedBio(data.bio || 'Hey there! I am using Cyphr Messenger');
      } else {
        // Use cached data
        setProfile({
          id: userId,
          name: userName || `User ${userPhone?.slice(-4)}`,
        ...
        setEditedName(userName || `User ${userPhone?.slice(-4)}`);
