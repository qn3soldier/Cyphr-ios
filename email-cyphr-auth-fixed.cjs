...
function createEmailAuthEndpoints(app, supabase, redis, JWT_SECRET) {
  ...
  app.post('/api/auth/send-email-otp', async (req, res) => {
    try {
      ...
      // TEST MODE: Skip actual email sending if no valid credentials
      const TEST_MODE = !process.env.EMAIL_USER || process.env.EMAIL_USER === 'noreply@cyphrmessenger.app';
      
      if (TEST_MODE) {
        console.log(`📧 TEST MODE: Email OTP for ${email}: ${otp} (hash: ${emailHash.substr(0,8)}...)`);
        res.json({ 
          success: true, 
          message: 'TEST MODE: Verification code generated (check server logs)',
          emailHash,
