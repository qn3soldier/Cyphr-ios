...
// AWS NATIVE VERSION - Using AWS SES
...
// AWS SES Client - NATIVE AWS SERVICE
...
Cyphr Messenger - Post-Quantum Secure Messaging
If you didn't request this code, please ignore this email.`,
      ...
      try {
        ...
        console.log(`📧 AWS SES EMAIL SENT: ${email} -> OTP: ${otp} -> MessageId: ${result.MessageId}`);
      } catch (sesError) {
        console.error('AWS SES Error:', sesError.message);
        // Fallback to console logging for development
        console.log(`📧 DEV FALLBACK - EMAIL OTP for ${email}: ${otp} (hash: ${emailHash.substr(0,8)}...)`);
        console.log('⚠️  AWS SES not configured - check IAM permissions and verified domains');
      }
  ...
  console.log('✅ AWS NATIVE Email & Cyphr ID authentication endpoints created');
  console.log('📧 Email service: AWS SES (native AWS integration)');
}
