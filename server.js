...
const redisAdapterPublisher = new ioredis_1.default({
    host: process.env.REDIS_HOST || '127.0.0.1',
    port: parseInt(process.env.REDIS_PORT || '6379'),
    connectTimeout: 10000,
    enableOfflineQueue: false
...
async function initializeServices() {
    try {
        const { default: FinalKyber1024 } = await Promise.resolve().then(() => require('./src/api/crypto/finalKyber1024.js'));
...
setTimeout(async () => {
    try {
        io.adapter((0, redis_adapter_1.createAdapter)(redisAdapterPublisher, redisAdapterSubscriber));
...
class SupabaseServerSingleton {
    ...
    static getAdminInstance() {
        ...
        return (0, supabase_js_1.createClient)(SUPABASE_URL, SERVICE_ROLE_KEY, {
            auth: {
                autoRefreshToken: false,
                persistSession: false,
                detectSessionInUrl: false
...
app.post('/api/auth/get-user-profile', async (req, res) => {
    try {
        const { userId } = req.body;
        if (!userId) {
            return res.status(400).json({ error: 'userId required' });
...
app.post('/api/auth/check-cyphr-id-availability', async (req, res) => {
    try {
